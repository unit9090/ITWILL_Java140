...
public class ListMain02 {
	...
	public static void main(String[] args) {
		...
		// 리스트 numbers에 [0, 100)(0 이상 100 미만) 범위의 정수 난수 20개를 저장.
		...
		// 홀수들만 저장할 LinkedList 변수(odds)를 선언, 객체 생성.
		LinkedList<Integer> odds = new LinkedList<>();
		
		// numbers의 정수들 중에서 홀수들만 odds에 저장.
		for(int x : numbers) {	// 리스트 numbers의 원소들을 처음부터 끝까지 순회하면서
			if(x % 2 != 0) {	// 원소가 홀수이면
				odds.add(x);	// 리스트 odds에 추가
			}
		}
		...
		for(int x : numbers) {	// 리스트 numbers의 원소들을 처음부터 끝까지 순회하면서
			if(x % 2 == 0) {	// 리스트의 원소가 짝수이면
				evenSquares.add(x * x);	// 원소의 제곱을 저장.
			}
		}
