...
/*
 *  변수 선언 위치:
 *  1. 필드: 클래스의 멤버로 선언하는 변수(멤버 변수). 접근 수식어를 사용할 수 있음.
 ...
 *  	- 파라미터: 메서드를 호출할 때 argument를 저장하기 위한 변수.
 *  	- 파라미터도 지역 변수의 한 종류.
 *  	- 지역변수는 static으로 선언할 수 없음!
 *  	- 접근 수식어(private, protected, public)를 사용할 수 없음!
 *  	- 지역 변수엔 final이라는 수식어만을 사용할 수 있다.
 *  
...
public class InnerMain01 {
	...
	private int x;	// 인스턴스 변수 = 생성된 객체(생성된 클래스에서만 접근 가능한 변수)	-> 메모리 Heap 영역
...
//		Enclosing.Nested nested = new Enclosing.Nested(2);	// -> import 문 없을 때
		Nested nested = new Enclosing.Nested(2);
		nested.printInfo();
	}
