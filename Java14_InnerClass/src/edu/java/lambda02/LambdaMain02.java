...
//		System.out.println(numbers);
		...
		List<Integer> oddes = numbers.stream()	// 리스트의 원소들이 순서대로 지나가는 통로
				.filter(x -> x % 2 == 1)		// 조건을 만족하는 원소들만 필터링
				.map(x -> x * x)				// 값을 다른 값으로 매핑(변환)
				.toList();						// 결과를 리스트로 저장.
		System.out.println(oddes);
		...
		// 1) 람다 표현식
		List<String> longLang = languages.stream()
				.filter(x -> x.length() >= 5)
				.map(x -> x.toLowerCase())
				.toList();
		System.out.println(longLang);
