...
// - seeter 메서드의 이름은 보통 필드 이름 앞에 set을 붙여서 만듦.
...
//		System.out.println(p1.age);		// 필드 값 읽기
		...
		// User 클래스
		User user = new User(110, "user123", "user123!");
		System.out.printf("회원번호: %d, 아이디: %s, 비밀번호: %s\n",
				user.getUserNo(), user.getUserId(), user.getPassword());
		
		user.setUserNo(220);
		user.setUserId("user220");
		user.setPassword("user220!");
		System.out.printf("회원번호: %d, 아이디: %s, 비밀번호: %s\n",
				user.getUserNo(), user.getUserId(), user.getPassword());
