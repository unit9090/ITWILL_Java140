...
// 클래스는 private와 (package)만 사용 가능하다.

// 클래스 멤버(필드, 생성자, 메소드)의 접근 제한 수식(access modifier)
// 1. private: 선언된 클래스 안에서만 직접 접근(사용) 가능. -> 다른 클래스에서는 보이지 않는다.
...
//		System.out.println("a = " + test.a);	=> not visible: private
		System.out.println("b = " + test.b);	// 수식어가 없는 경우 같은 패키지 범위에서는 보임.
...
//		test.a = 100;	=> not visible
		...
		// 생성자를 private으로 선언하면 다른 클래스에서는 생성자를 호출할 수 없기 때문에 객체를 생성할 수 없음.
		// ex> Math
