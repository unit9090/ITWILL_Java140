...
// 인터페이스(interface):
// 사용 목적: 팀/회사 간의 분업/협업을 하기 위해서 메서드(기능)들의 프로토타입(prototype, signature(=> 바디없는 메서드))을 약속(정의)하는 것.
// 인터페이스가 가질 수 있는 멤버들:
...
public class InterfaceMain01 {
	...
	public static void main(String[] args) {
		...
		// -> import static 문장을 사용하면 'DatabaseModule.DB_VERSION'를 간단히 DB_VERSION으로 사용할 수 있음.
		
		// DatabaseModule.DB_VERSION = 2;	=> final이기 때문에 변경 불가.
		
		// new DatabaseModule();
		// Cannot instantiate the type(class) DatabaseModule:
		// 인터페이스는 객체를 생성할 수 없음.
