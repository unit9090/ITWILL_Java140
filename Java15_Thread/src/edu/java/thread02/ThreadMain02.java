...
public class ThreadMain02 {
	...
	public static void main(String[] args) {
		 ...
		 *  2. 구현 클래스에서 Runnable 인터페이스의 run 메소드를 재정의 - 스레드가 할 일
		 *  3. Thread 생성자를 호출할 때 Runnable 구현 객체를 argument로 전달.
		 *  	new Thread(Runnable runnable) 생성자 호출. -> 스레드 객체 생성.
		 *  4. 스레드 객체의 start 메서드 호출 -> 스레드 실행 -> run 자동 실행.
		 */
		...
		long start = System.currentTimeMillis();	// 스레드 시작 전 시간 기록
		...
		long end = System.currentTimeMillis();	// 스레드 종료 시간 기록.
